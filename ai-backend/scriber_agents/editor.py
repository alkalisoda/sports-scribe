...
class Editor:
    ...
    async def validate_article(
        ...
        try:
            ...
            validation_tasks = [
                ...
                self._validate_player_performance(
                    text, validation_data["player_performance"]
                ),
                ...
                self._validate_background_info(
                    text, validation_data["background_info"]
                ),
            ...
            for i, result in enumerate(validation_results):
                if isinstance(result, Exception):
                    ...
                    if result.get("errors_found", False):
                        comprehensive_results["total_errors"] += len(
                            result.get("errors", [])
                        )
                        comprehensive_results["all_errors"].extend(
                            result.get("errors", [])
                        )

            logger.info(
                f"Validation completed. Total errors found: {comprehensive_results['total_errors']}"
            )
    ...
    async def edit_with_facts(
        ...
        try:
            ...
            validation_results = await self.validate_article(
                text, game_info, research_insights
            )
    ...
    def _prepare_validation_data(
        self, game_info: dict[str, Any], research_insights: dict[str, Any] | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            validation_data = {
                ...
                "player_performance": self._prepare_player_performance_data(
                    base_game_data, research_insights
                ),
                ...
                "background_info": self._prepare_background_info_data(
                    base_game_data, research_insights
                ),
                "terminology": self._prepare_terminology_data(
                    base_game_data, research_insights
                ),
            }
    ...
    def _prepare_score_process_data(
        self, base_game_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _prepare_player_performance_data(
        self, base_game_data: dict[str, Any], research_insights: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    def _prepare_substitution_data(
        self, base_game_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _prepare_disciplinary_data(
        self, base_game_data: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        return {
            "events": base_game_data.get("events", []),
            "teams": base_game_data.get("teams", {}),
            "fixture": {"referee": base_game_data.get("referee")},
        }

    def _prepare_background_info_data(
        self, base_game_data: dict[str, Any], research_insights: dict[str, Any] | None = None
    ) -> dict[str, Any]:
    ...
    async def _validate_score_process(
        self, text: str, game_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _validate_player_performance(
        self, text: str, game_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _validate_substitutions(
        self, text: str, game_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _validate_disciplinary(
        self, text: str, game_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def _validate_background_info(
        self, text: str, game_data: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    async def edit_with_terms(self, text: str, game_info: dict[str, Any] | None = None) -> str:
    ...
    def validate_editing_result(
        ...
        try:
            validation_result = {
                ...
                "preserves_structure": self._check_structure_preservation(
            ...
            length_ratio = (
                abs(validation_result["length_change"])
                / validation_result["original_length"]
            )
            if length_ratio > 0.1:
                validation_result["warning"] = (
                    f"Significant length change detected: {validation_result['length_change']} words"
                )
    ...
    def _check_structure_preservation(
        ...
        try:
            ...
            original_has_structure = all(
                element in original_text for element in structure_elements
            )
            edited_has_structure = all(
                element in edited_text for element in structure_elements
            )
