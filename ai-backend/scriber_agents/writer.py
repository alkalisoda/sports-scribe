...
from .narrative_planner import NarrativePlan
...
class StylizedWriter:
    """
    WriterAgent v2: Apply narrative plan to produce a stylistic, emotionally engaging article.
    Used after narrative plan is generated by NarrativePlanner.
    """
    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.agent = Agent(
            name="StylizedWriter",
            description="A agent that writes a stylized article",
    ...
    def get_instructions(self):
        ...
        You will be given a factual article and a narrative plan.
        You will need to rewrite the factual article using the narrative plan to make it vivid, emotional, and immersive.
        """

    async def write_with_narrative(self, basic_factual_article: str, plan: NarrativePlan) -> str:
        """
        Use the factual article and a NarrativePlan to generate a stylized, dramatic match recap.
        """
        logger.info("Rewriting article with narrative structure")

        guidance = self._build_guidance(plan)

        prompt = f"""
        You are a sports journalist.
        Rewrite the following factual article using the narrative blueprint below to make it vivid, emotional, and immersive:

        === FACTUAL ARTICLE ===
        {basic_factual_article}

        === NARRATIVE BLUEPRINT ===
        {guidance}

        Focus on tone, pacing, structure, character development, emotional resonance, and storytelling. 
        ...
        try:
            ...
            logger.warning(f"Stylized writing failed, returning original article: {e}")
            return basic_factual_article

    def _build_guidance(self, plan: NarrativePlan) -> str:
        structure = "\n".join([f"{i+1}. {step}" for i, step in enumerate(plan.narrative_flow)])
        characters = "\n".join([
            f"- {c.name} ({c.role_type}): {c.arc_description}" for c in plan.main_characters
        ])
        return f"""
        Narrative Arc: {plan.narrative_arc.value}
        Theme: {plan.central_theme}
        Tone: {plan.tone_direction}
        Pacing: {plan.pacing_notes}
        Opening: {plan.opening_hook}
        ...
        Emotions: {', '.join(plan.emotional_peaks)}
        Tension Builders: {', '.join(plan.tension_builders)}
        Surprises: {', '.join(plan.surprise_elements)}
        Human Angles: {', '.join(plan.human_interest_angles)}
        """
