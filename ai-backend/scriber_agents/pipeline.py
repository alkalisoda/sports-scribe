...
class AgentPipeline:
    ...
    async def generate_game_recap(self, game_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if raw_game_data.get("errors") and len(raw_game_data.get("errors", [])) > 0:
                logger.warning(
                    f"[PIPELINE] Data collection had errors: {raw_game_data['errors']}"
                )
                if raw_game_data.get("results", 0) == 0:
                    raise ValueError(
                        f"No data available for game {game_id}: {raw_game_data['errors']}"
                    )

            # Log raw data information
            logger.info("[PIPELINE-DATA] Raw game data collected:")
            logger.info(f"[PIPELINE-DATA]   Type: {type(raw_game_data)}")
            logger.info(
                f"[PIPELINE-DATA]   Keys: {list(raw_game_data.keys()) if isinstance(raw_game_data, dict) else 'Not a dict'}"
            )
            if isinstance(raw_game_data, dict):
                logger.info(
                    f"[PIPELINE-DATA]   Response count: {raw_game_data.get('response', [])}"
                )
                logger.info(
                    f"[PIPELINE-DATA]   Errors: {raw_game_data.get('errors', [])}"
                )
                logger.info(
                    f"[PIPELINE-DATA]   Results: {raw_game_data.get('results', 0)}"
                )

            logger.info("[PIPELINE] Raw game data collected successfully")

            # Step 1.5: Extract compact game data format
            logger.info("[PIPELINE] Step 1.5: Extracting compact game data format")
            try:
                ...
                raise ValueError(f"Failed to extract compact game data: {e}") from e

            # Log compact data information
            logger.info("[PIPELINE-DATA] Compact game data extracted:")
            ...
            if isinstance(compact_game_data, dict) and "error" not in compact_game_data:
                ...
                logger.warning(
                    f"[PIPELINE-DATA]   Compact data error: {compact_game_data.get('error', 'Unknown error')}"
                )

            # Log team and player info for enhanced data collection
            logger.info("[PIPELINE-DATA] Team info extracted:")
            ...
            if isinstance(team_info, dict) and "error" not in team_info:
                ...
                logger.info(
                    f"[PIPELINE-DATA]   League: {team_info.get('league', {}).get('name', 'Unknown')}"
                )
            else:
                logger.warning(
                    f"[PIPELINE-DATA]   Team info error: {team_info.get('error', 'Unknown error')}"
                )

            logger.info("[PIPELINE-DATA] Player info extracted:")
            ...
            if isinstance(player_info, dict) and "error" not in player_info:
                ...
                logger.warning(
                    f"[PIPELINE-DATA]   Player info error: {player_info.get('error', 'Unknown error')}"
                )

            logger.info(
                "[PIPELINE] Compact game data and team/player information extracted successfully"
            )

            # Step 1.6: Collect enhanced team and player data using data collector
            logger.info("[PIPELINE] Step 1.6: Collecting enhanced team and player data")
            ...
            enhanced_player_data = await self.collect_enhanced_player_data(
                player_info, season
            )

            # Log enhanced data collection
            logger.info("[PIPELINE-DATA] Enhanced team data collected:")
            logger.info(f"[PIPELINE-DATA]   Type: {type(enhanced_team_data)}")
            if (
                isinstance(enhanced_team_data, dict)
                and "error" not in enhanced_team_data
            ):
                ...
                logger.warning(
                    f"[PIPELINE-DATA]   Enhanced team data error: {enhanced_team_data.get('error', 'Unknown error')}"
                )

            logger.info("[PIPELINE-DATA] Enhanced player data collected:")
            logger.info(f"[PIPELINE-DATA]   Type: {type(enhanced_player_data)}")
            if (
                isinstance(enhanced_player_data, dict)
                and "error" not in enhanced_player_data
            ):
                enhanced_key_players = len(
                    enhanced_player_data.get("enhanced_key_players", [])
                )
                sample_players = len(
                    enhanced_player_data.get("sample_players_detailed", [])
                )
                logger.info(
                    f"[PIPELINE-DATA]   Enhanced key players: {enhanced_key_players}"
                )
                logger.info(
                    f"[PIPELINE-DATA]   Sample players detailed: {sample_players}"
                )
            else:
                logger.warning(
                    f"[PIPELINE-DATA]   Enhanced player data error: {enhanced_player_data.get('error', 'Unknown error')}"
                )

            logger.info(
                "[PIPELINE] Enhanced team and player data collected successfully"
            )

            # Step 2: Research and generate storylines
            logger.info(
                "[PIPELINE] Step 2: Conducting research and generating storylines"
            )

            # Step 2.1: Analyze game data for storylines (using compact data)
            logger.info("[PIPELINE] Step 2.1: Analyzing game data for storylines")
            game_analysis = await self.researcher.get_storyline_from_game_data(
                compact_game_data
            )
            logger.info(
                f"[PIPELINE-DATA] Game analysis storylines: {len(game_analysis) if isinstance(game_analysis, list) else 'Not a list'}"
            )

            # Step 2.2: Analyze historical context between teams
            logger.info(
                "[PIPELINE] Step 2.2: Analyzing historical context between teams"
            )
            historical_context = await self.researcher.get_history_from_team_data(
                enhanced_team_data
            )
            logger.info(
                f"[PIPELINE-DATA] Historical context storylines: {len(historical_context) if isinstance(historical_context, list) else 'Not a list'}"
            )

            # Step 2.3: Analyze individual player performances (using compact data)
            logger.info("[PIPELINE] Step 2.3: Analyzing individual player performances")
            player_performance_analysis = (
                await self.researcher.get_performance_from_player_game_data(
                    enhanced_player_data, compact_game_data
                )
            )
            logger.info(
                f"[PIPELINE-DATA] Player performance storylines: {len(player_performance_analysis) if isinstance(player_performance_analysis, list) else 'Not a list'}"
            )
            ...
            logger.info("[PIPELINE-DATA] Comprehensive research data:")
            logger.info(f"[PIPELINE-DATA]   Type: {type(comprehensive_research_data)}")
            logger.info(
                f"[PIPELINE-DATA]   Keys: {list(comprehensive_research_data.keys())}"
            )
            logger.info(
                f"[PIPELINE-DATA]   Game analysis storylines: {len(game_analysis)}"
            )
            logger.info(
                f"[PIPELINE-DATA]   Historical context: {len(historical_context)}"
            )
            logger.info(
                f"[PIPELINE-DATA]   Player performance: {len(player_performance_analysis)}"
            )

            logger.info(
                f"[PIPELINE] Research completed, generated {len(game_analysis)} game storylines, {len(historical_context)} historical context items, {len(player_performance_analysis)} player performance items"
            )

            # Step 3: Generate article content
            logger.info("[PIPELINE] Step 3: Generating article content")
            ...
            logger.info("[PIPELINE-DEBUG] Data passed to writer:")
            logger.info(
                f"[PIPELINE-DEBUG]   game_info type: {type(game_info)}, keys: {list(game_info.keys()) if isinstance(game_info, dict) else 'Not a dict'}"
            )
            logger.info(
                f"[PIPELINE-DEBUG]   research type: {type(research_for_writer)}, keys: {list(research_for_writer.keys()) if isinstance(research_for_writer, dict) else 'Not a dict'}"
            )
            ...
            logger.info("[PIPELINE-DATA] Generated article:")
            logger.info(f"[PIPELINE-DATA]   Type: {type(article_content)}")
            logger.info(
                f"[PIPELINE-DATA]   Length: {len(article_content) if isinstance(article_content, str) else 'Not a string'}"
            )
            if isinstance(article_content, str):
                logger.info(f"[PIPELINE-DATA]   Preview: {article_content[:200]}...")

            logger.info("[PIPELINE] Article content generated successfully")

            # Step 4: Edit and fact-check the article
            logger.info("[PIPELINE] Step 4: Editing and fact-checking article")
            original_article = article_content

            # Step 4.1: Fact-checking with research insights
            logger.info(
                "[PIPELINE] Step 4.1: Fact-checking article with research insights"
            )
            ...
            logger.info("[PIPELINE] Step 4.2: Terminology checking article")
            edited_article = await self.editor.edit_with_terms(
                fact_checked_article, compact_game_data
            )

            # Validate editing results
            validation_result = self.editor.validate_editing_result(
                original_article, edited_article
            )
            ...
            logger.info("[PIPELINE] Article editing completed successfully")

            # Step 5: Return results
            pipeline_duration = (datetime.now() - pipeline_start_time).total_seconds()
            logger.info(
                f"[PIPELINE] Game recap generation completed in {pipeline_duration:.2f} seconds"
            )
            ...
            return {
                ...
                "editing_metadata": {
                    ...
                    "preserves_structure": validation_result.get(
                ...
                "data_format_metadata": {
                    ...
                    "compact_data_structure": {
                        ...
                        "events": (
                            len(compact_game_data.get("events", []))
                            if isinstance(compact_game_data, dict)
                            else 0
                        ),
                        "players": (
                            len(compact_game_data.get("players", []))
                            if isinstance(compact_game_data, dict)
                            else 0
                        ),
                        "statistics_teams": (
                            len(compact_game_data.get("statistics", []))
                            if isinstance(compact_game_data, dict)
                            else 0
                        ),
                        "lineups_teams": (
                            len(compact_game_data.get("lineups", []))
                            if isinstance(compact_game_data, dict)
                            else 0
                        ),
                    },
            ...
            pipeline_duration = (datetime.now() - pipeline_start_time).total_seconds()
            logger.error(
                f"[PIPELINE] Error generating game recap for {game_id} after {pipeline_duration:.2f} seconds: {e!s}"
            )
            return {
                "success": False,
                "game_id": game_id,
    ...
    async def _collect_game_data(self, game_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("[PIPELINE] Game data collected successfully")
            return data
        except Exception as e:
            ...
            return {
                ...
                "errors": [f"Failed to collect game data: {e!s}"],
                "results": 0,
    ...
    def extract_team_info(self, raw_game_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"[PIPELINE] Successfully extracted team info for {home_team_info['name']} vs {away_team_info['name']}"
            )
            return team_info
    ...
    def extract_player_info(self, raw_game_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"[PIPELINE] Successfully extracted player info for {len(all_players)} players"
            )
            return player_info

        except Exception as e:
            logger.error(f"[PIPELINE] Error extracting player info: {e}")
            return {"error": f"Failed to extract player info: {e!s}"}

    def _identify_key_players(
        self, all_players: dict[str, Any], events: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
    ...
    async def collect_enhanced_team_data(
        ...
        try:
            ...
            if home_team_id:
                try:
                    logger.info(
                        f"[PIPELINE] Collecting detailed data for home team {home_team_id}"
                    )
                    home_team_detailed = await self.collector.collect_team_data(
                        str(home_team_id)
                    )
                    enhanced_team_data["enhanced_data"][
                        "home_team_detailed"
                    ] = home_team_detailed
                    logger.info(
                        "[PIPELINE] Successfully collected home team detailed data"
                    )
                except Exception as e:
                    logger.warning(
                        f"[PIPELINE] Failed to collect home team detailed data: {e}"
                    )
                    enhanced_team_data["enhanced_data"]["home_team_detailed"] = {
                        "error": str(e)
                    }
            ...
            if away_team_id:
                try:
                    logger.info(
                        f"[PIPELINE] Collecting detailed data for away team {away_team_id}"
                    )
                    away_team_detailed = await self.collector.collect_team_data(
                        str(away_team_id)
                    )
                    enhanced_team_data["enhanced_data"][
                        "away_team_detailed"
                    ...
                    logger.info(
                        "[PIPELINE] Successfully collected away team detailed data"
                    )
                except Exception as e:
                    logger.warning(
                        f"[PIPELINE] Failed to collect away team detailed data: {e}"
                    )
                    enhanced_team_data["enhanced_data"]["away_team_detailed"] = {
                        "error": str(e)
                    }
    ...
    async def collect_enhanced_player_data(
        ...
        try:
            ...
            if not season:
                logger.warning(
                    "[PIPELINE] Season not found, cannot collect enhanced player data."
                )
                return {"error": "Season not available in raw game data"}

            for _i, player in enumerate(key_players[:5]):  # Limit to top 5 key players
                player_id = player.get("id")
                if player_id:
                    try:
                        logger.info(
                            f"[PIPELINE] Collecting detailed data for key player {player_id} ({player.get('name', 'Unknown')})"
                        )
                        player_detailed = await self.collector.collect_player_data(
                            str(player_id), str(season)
                        )
                        ...
                        logger.info(
                            f"[PIPELINE] Successfully collected detailed data for player {player_id}"
                        )
                    except Exception as e:
                        logger.warning(
                            f"[PIPELINE] Failed to collect detailed data for player {player_id}: {e}"
                        )
                        enhanced_player = player.copy()
            ...
            for player in home_players[:2]:
                ...
                if player_id:
                    try:
                        logger.info(
                            f"[PIPELINE] Collecting sample data for home player {player_id}"
                        )
                        player_detailed = await self.collector.collect_player_data(
                            str(player_id), str(season)
                        )
                        ...
                        logger.warning(
                            f"[PIPELINE] Failed to collect sample data for home player {player_id}: {e}"
                        )
            ...
            for player in away_players[:2]:
                ...
                if player_id:
                    try:
                        logger.info(
                            f"[PIPELINE] Collecting sample data for away player {player_id}"
                        )
                        player_detailed = await self.collector.collect_player_data(
                            str(player_id), str(season)
                        )
                        ...
                        logger.warning(
                            f"[PIPELINE] Failed to collect sample data for away player {player_id}: {e}"
                        )

            enhanced_player_data["sample_players_detailed"] = sample_players

            logger.info(
                f"[PIPELINE] Enhanced player data collection completed. Key players: {len(enhanced_key_players)}, Sample players: {len(sample_players)}"
            )
            return enhanced_player_data

        except Exception as e:
            logger.error(f"[PIPELINE] Error collecting enhanced player data: {e}")
            return {"error": f"Failed to collect enhanced player data: {e!s}"}
    ...
    def extract_compact_game_data(
        self, raw_game_data: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("[PIPELINE] Successfully extracted compact game data")
            logger.info("[PIPELINE-DATA] Compact data structure:")
            ...
            logger.error(f"[PIPELINE] Error extracting compact game data: {e}")
            return {"error": f"Failed to extract compact game data: {e!s}"}
    ...
    def _extract_match_info(self, fixture_data: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            match_info = {
                ...
                "score": {"fulltime": score.get("fulltime", {})},
            }
            ...
            return {"error": f"Failed to extract match info: {e!s}"}

    def _extract_events(
        self, fixture_data: dict[str, Any], max_events: int = 20
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            sorted_events = sorted(
                events, key=lambda x: x.get("time", {}).get("elapsed", 0)
            )
            ...
            for event in limited_events:
                ...
                # Special handling for card events to exclude from player performance
                    ...
                    extracted_event = {
                        ...
                        "time": {"elapsed": event.get("time", {}).get("elapsed")},
                        "player": {"name": event.get("player", {}).get("name")},
                        "team": {"name": event.get("team", {}).get("name")},
                    }
    ...
    def _process_goal_event(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            return {
                ...
                "assist": (
                    event.get("assist", {}).get("name") if event.get("assist") else None
                ),
                "minute": event.get("time", {}).get("elapsed"),
            }
        except Exception as e:
        ...
        try:
            ...
            for lineup in lineups:
                extracted_lineup = {
                    ...
                    "coach": {"name": lineup.get("coach", {}).get("name")},
                    "formation": lineup.get("formation"),
                    "startXI": lineup.get("startXI", []),
                    "substitutes": lineup.get("substitutes", []),
                }
