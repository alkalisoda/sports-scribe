        ...
        "lineups": [
            {
                "team": "Manchester United",
                "formation": "4-3-3",
                "startXI": ["Onana", "Dalot", "Varane", "Evans", "Shaw", "Casemiro", "Mainoo", "Fernandes", "Rashford", "Højlund", "Garnacho"]
            },
            {
                "team": "Liverpool",
                "formation": "4-3-3",
                "startXI": ["Alisson", "Alexander-Arnold", "Van Dijk", "Konaté", "Robertson", "Szoboszlai", "Mac Allister", "Jones", "Salah", "Núñez", "Díaz"]
            }
        ]
    }
...
async def test_narrative_planner():
    ...
    try:
        ...
        narrative_selection = await narrative_planner.select_narrative(compact_data, research_data)
        
        logger.info("Narrative selection completed successfully")
        logger.info(f"Primary narrative: {narrative_selection.get('primary_narrative', 'Unknown')}")
        logger.info(f"Storytelling focus: {narrative_selection.get('storytelling_focus', 'Unknown')}")
        ...
        strength_analysis = await narrative_planner.analyze_narrative_strength(narrative_selection)
        ...
        logger.error("Make sure you're running this from the correct directory and the modules are available")
        return False
